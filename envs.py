...
#Environment specific wrappers for reward
import math
...
class BBReward_wrapper(gym.RewardWrapper):
    def reset(self, **kwargs):
        self.lives = 3
        self.score = 0
        self.tot_reward = 0
    ...
    def step(self, action):
        observation, reward, done, info = self.env.step(action)
        points = info['score'] - self.score
        step_lives = info["lives"] - self.lives
        self.score = info['score']
        self.lives = info["lives"]
        reward = step_lives + ((sigmoid(points / 100)) - 0.5)
        self.tot_reward += reward
        info["score"] = self.tot_reward
